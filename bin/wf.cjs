...
const envPath = process.env.WF_BIN_PATH
const scriptDir = path.dirname(fs.realpathSync(__filename))
const cached = path.join(scriptDir, ".wf")
const platform = { darwin: "darwin", linux: "linux", win32: "windows" }[os.platform()] || os.platform()
const arch = { x64: "x64", arm64: "arm64", arm: "arm" }[os.arch()] || os.arch()
const base = "@workflowfiesta/cli-" + platform + "-" + arch
const binary = platform === "windows" ? "wf.exe" : "wf"
...
if (!resolved) {
  console.error(
    "It seems that your package manager failed to install the right WorkflowFiesta CLI package. Try manually installing " +
